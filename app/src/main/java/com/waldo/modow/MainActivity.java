...
public class MainActivity extends Activity {
    ...
    private void showCompleteDialog(AppDb.Habit h, LocalDate day) {
        AlertDialog dialog=styledBuilder()
                ...
                .setPositiveButton("Sí, cumplido",(d,w)->{db.complete(h.id(),day);showToday();})
                .create();
    ...
    private void showConfig(){ showConfig(-1,0); }

    private void showConfig(long animatedHabitId,int moveDelta){
    ...
    private void editDialog(AppDb.Habit h){
        ...
        ArrayAdapter<String> adapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item,days){
            private TextView decorate(View view,boolean dropdown){
                TextView tv=(TextView)view; tv.setTextColor(TEXT); tv.setTextSize(15); tv.setPadding(dp(12),dp(10),dp(12),dp(10));
                tv.setBackgroundColor(dropdown?DIALOG_BG:Color.TRANSPARENT); return tv;
            }
            @Override public View getView(int position,View convertView,ViewGroup parent){ return decorate(super.getView(position,convertView,parent),false); }
            @Override public View getDropDownView(int position,View convertView,ViewGroup parent){ return decorate(super.getDropDownView(position,convertView,parent),true); }
        };
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        day.setAdapter(adapter); day.setSelection(initialWeekday-1);
